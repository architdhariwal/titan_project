...
describe('Card Component', () => {
  ...
  it('uses placeholder image when no image is provided', () => {
    ...
    expect(img).toHaveAttribute('src', 'https://via.placeholder.com/360x360?text=No+Image');
  });

  it('truncates long titles', () => {
    const propsWithLongTitle = { ...mockProps, title: 'This is a very long product title that should be truncated' };
    render(<Card {...propsWithLongTitle} />);

    const titleElement = screen.getByText('This is a very long product title that should be truncated');
